...
##### Second approach:
Nonmissing genes only (621 genes left); 
...
```
We see here that the two clusters are distinct in terms of the proportion of superficial and muscle-invasive tumor. we only have 6 misclassification (5 in superficial and 1 in muscle invasive). Note also that all the 'squamous' histological type tumor are in the same cluster. This is consistent with the results reported by the paper which also have 6 similar misclassification. This shows that at least in this example, the result is not sensitive to the choice of filtering, imputation, and clustering algorithm. We can still clearly distinguish the superficial and the muscle invasive tumor based on the unsupervised cluster.
...
Here we also have 6 misclassification (4 in superficial and 2 in muscle invasive). This shows that even with just 10 prinicipal components based on the 621 non-missing genes compared to the 2,675 genes the paper uses after filtering, we can still have two distinct clusters with the same misclassifications.
